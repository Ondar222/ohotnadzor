...
export async function getServerSideProps({ res }) {
  ...
  const Menu = await fetch(`${process.env.APIpath}/api/menu`,
  AuthHeaders)
  .then((res) => res.json())
  .then((res) => res.data)

  const sitemap = generateSiteMap(posts, aboutPages, activityPages, civilservicePages, menu);
...
function generateSiteMap(posts, about, activity, civilservice, menu) {
  return `<?xml version="1.0" encoding="UTF-8"?>
   <urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
     ...
     <url>
       <loc>http://ohotnadzor.rtyva.ru/menu</loc>
     </url>
     ${menu.map(({ title }) => {
    return `
        <url>
           <loc>${`${EXTERNAL_DATA_URL}/menu/${title}`}</loc>
       </url>
        `
  })
      .join('')}
