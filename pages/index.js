...
import OhotaMenu, { AboutPageLayout } from '../components/OhotaMenu';
import Link from 'next/link';
import Image from 'next/image';
import Script from 'next/script';
import { useEffect } from 'react'
import { Widget } from '../classes/pos.class'
        ...
        <MainPageLayout>
            ...
            <div className="w-full flex flex-row gap-3">
                ...
                <div className="xs:hidden md:hidden lg:flex xs:flex-row md:flex-col gap-3 justify-start xs:w-full md:w-1/5 min-w-40 min-h-40">
                    <AboutPageLayout />
                    <div className="flex xs:flex-col md:flex-col mt-7 mb-7 justify-start xs:w-full md:w-5/5 min-w-40 min-h-40 gap-5">
                        {
                            links?.pinned_links?.map((item) => {
                                return (
                                    <Link href={item.url} key={item.id} className="min-w-60 min-h-60 cursor-pointer ">
                                        <a>
                                            <Image
                                                ...
                                                layout={"responsive"}
                                                objectFit={"cover"}
                                                className="md:w-5/5 min-w-40 min-h-40 dark:grayscale cursor-pointer"
                                            />
                                        </a>
                                    </Link>
                                )
